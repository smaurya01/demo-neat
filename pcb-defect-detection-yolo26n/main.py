...
Runs the compiled MLA model pack on the DevKit via the public `pyneat` runtime
over a folder of PCB images, drawing class-colored boxes + labels at the original
resolution. Invoke on the board through `dk` (see scripts/run_dk.sh).
...
def main() -> int:
    ...
    if not model_pack.exists():
        logger.error("compiled pack not found: %s (build it with scripts/compile_model.sh)", model_pack)
        return 2
