...
>
> All ten models have already been compiled. The **original weights + ONNX export + compiled SiMa
> `.tar.gz`** are published as a download — drop the archive into an app's `assets/models/` and run.
>
> **→ [`assets/models/README.md`](assets/models/README.md)** — download link, manifest, and exactly
> which archive each app expects.
>
...
**Compile all ten unattended:** `./compile_all.sh` — serial (one at a time), ~2 h, collects every
model's artifacts into `assets/models/<id>/`. Progress goes to `compile_all.log`.

**Everything here is reproducible from scratch.** The repo holds only the recipe (`models.yaml`,
`compile/`) and the calibration images (`assets/calibration/`). Weights, graphs and compiled archives
are downloaded/generated by the flow and are **not in git** — clone, run the four steps, get identical
archives.
...
Measured **end-to-end** per model — all four steps (download + export + surgery + INT8 compile +
validate), from a full `compile_all.sh` run on the SDK container:
...
**All 10 models ≈ 2 h**, serial. One compile at a time — concurrent compiles OOM. Times scale with
host CPU; treat them as ratios, not promises.
