...
def model_spec() -> tuple[str, str]:
    name = project_name()
    if name == "single-stream-yolov8n-seg":
        return "yolov8n-seg", "yolo_v8n_seg_mpk.tar.gz"
    if name == "single-stream-yolo26n":
        return "yolo26n", "yolo26n-det-bf16-mla_tess-b1.tar.gz"
    if name == "single-stream-yolo-yolov8m":
        return "yolov8m", "yolo_v8m_mpk.tar.gz"
    return "yolov8n", "yolo_v8n_mpk.tar.gz"
...
def resolve_model_path(cfg: Config) -> str:
    ...
    models_dir = Path(cfg.models_dir) if cfg.models_dir else Path(__file__).resolve().parent / "assets" / "models"
...
def draw_boxes(frame, boxes: list[dict], min_score: float) -> int:
    ...
    for box in boxes:
        ...
        label = f"{class_label(box['class_id'])} {box['score']:.2f}"
...
def draw_boxes_on_nv12(nv12, width: int, height: int, boxes: list[dict], min_score: float) -> int:
    ...
    for box in boxes:
        ...
        if x2 <= x1 or y2 <= y1:
            continue
        draw_y_line(y_plane, x1, x2, y1, y_value, thickness)
        draw_y_line(y_plane, x1, x2, y2, y_value, thickness)
        draw_y_col(y_plane, x1, y1, y2, y_value, thickness)
        draw_y_col(y_plane, x2, y1, y2, y_value, thickness)
        draw_uv_rect(uv_plane, x1, y1, x2, y2, u_value, v_value)
        visible += 1
    return visible
...
def run(cfg: Config) -> int:
    ...
    graph = build_inference_graph(cfg, width, height, fps)
    ...
    inference_run = graph.build(run_options)
    ...
    try:
        while cfg.frames <= 0 or processed < cfg.frames:
            sample = inference_run.pull("output", 20000)
            if sample is None:
                print("[warn] timed out waiting for model output", file=sys.stderr)
                continue
            frame_tensor = frame_tensor_from_sample(sample)
            tensors = result_tensors_from_sample(sample)
            ...
            processed += 1
            print(f"frame={processed} detections={len(detections)} visible={visible}", flush=True)
            time.sleep(0)
    finally:
        inference_run.close()
        video_run.close()
