...
> (captured on the board 2026-07-09). Bring-your-own compilation is a **separate,
> host-side Model Compiler step**, documented by SiMa as the `llima-compile` tool
> (see [genai-llima/compilation_genai](https://developer.sima.ai/software/genai-llima/compilation_genai)).
> Nobody on this team has executed `llima-compile`, and it is not part of
> `/workspace/core`. Where a fact below comes from the official docs rather than a
> command we ran, it is marked **[docs]**. Where it comes from Neat core source it
> is marked **[core]**. Treat unverified specifics as "confirm with the LLiMa /
> Model Compiler team" rather than as tested truth.
...
- **Is the model already in the catalog?** Run `llima search` (cheap, safe) and
  check `<scratchpad>/llima_ground_truth.md`. If a suitable `...-a16w4` /
...
| Answer | Verdict |
| --- | --- |
| Base architecture is one of the **supported families** [docs]: Llama 2 / 3.1 / 3.2, Gemma 1 / 2 / 3 / 4, Phi-3.5, Qwen2.5 / Qwen3 (and `-VL`), Mistral, LFM2 / LFM2.5 (and `-VL`); ASR = Whisper | **YES** — proceed |
...
- **Pre-quantized compressed-tensors (GPTQ / AWQ)** → pipeline: SOURCE_TO_QUANT → COMPILE
  - **must use symmetric quantization** [docs] — asymmetric pre-quant is not accepted
...
A Hugging Face model directory must contain [docs]:
...
- **Host compile budget** — `llima-compile` runs on a host/workstation, not the
  board. The quantize step is heavy and a **GPU is recommended** [docs]. This is
  where big models are actually processed.
- **Board runtime budget** — the DevKit root FS has only **~5.9 GB free of 14 GB**
  (`df -h /`, 2026-07-09). Deployed weights land under
  `/media/nvme/llima/models/<id>` (nvme, larger than `/`), but **always run
  `df -h /` and `df -h /media/nvme` before deploying.**
...
- Default max context is **1024 tokens**, configurable at compile time [docs].
...
**[core: `/workspace/core/src/genai/GenAIInternal.cpp`, `inspect_model_directory`]**.
...
- For an **image-capable VLM**, `vlm_config.json` must carry non-null `vm_cfg`,
  ...
  runtime sets `accepts_image()` **[core]**. A VLM compiled without these loads as
...
[docs]; deployment reorganizes `sima_files` into the `devkit/` + `elf_files/`
...
Step 0  in catalog? ----------------- yes --> pull it, done (no compile)
                       no
Step 1  supported family? ----------- no  --> NOT LLiMa-able (ask LLiMa team)
                       yes
...
**No** = any hard gate fails; document why and route to the LLiMa / Model
Compiler team rather than guessing.
