...
std::filesystem::path app_dir_from_argv0(const char* argv0) {
  auto dir = std::filesystem::absolute(argv0).parent_path();
  if (std::filesystem::exists(dir / "config" / "default.conf")) {
    return dir;
  }
  if (std::filesystem::exists(dir.parent_path() / "config" / "default.conf")) {
...
std::filesystem::path resolve_app_path(const std::filesystem::path& app_dir,
...
std::string default_config_path(const char* argv0) {
  return (app_dir_from_argv0(argv0) / "config" / "default.conf").string();
}
...
void usage(const char* argv0) {
  std::cout
      << "Usage: " << argv0 << " [options]\n\n"
      << "Options:\n"
      << "  --config <path>           Key=value config file (default: app config/default.conf)\n"
...
Config parse_args(int argc, char** argv) {
  Config cfg;
  const auto app_dir = app_dir_from_argv0(argv[0]);
  ...
  if (!cfg.models_dir.empty()) {
    cfg.models_dir = resolve_app_path(app_dir, cfg.models_dir).string();
  }
