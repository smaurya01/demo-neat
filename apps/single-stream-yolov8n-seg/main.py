...
def mask_rect_for_frame_rect(frame_rect: tuple[int, int, int, int], frame_shape, mask_shape) -> tuple[int, int, int, int]:
    ...
    x1 = max(0, min(mask_w - 1, int(np.floor(to_mask_x(fx1)))))
    y1 = max(0, min(mask_h - 1, int(np.floor(to_mask_y(fy1)))))
    x2 = max(x1 + 1, min(mask_w, int(np.ceil(to_mask_x(fx2)))))
    y2 = max(y1 + 1, min(mask_h, int(np.ceil(to_mask_y(fy2)))))
    return x1, y1, x2, y2


def project_letterbox_mask_roi(mask, frame, rect: tuple[int, int, int, int]):
    x1, y1, x2, y2 = rect
    mx1, my1, mx2, my2 = mask_rect_for_frame_rect(rect, frame.shape, mask.shape)
    return cv2.resize(mask[my1:my2, mx1:mx2], (x2 - x1, y2 - y1), interpolation=cv2.INTER_LINEAR)
...
def overlay_segmentation(frame, detections: list[dict], min_score: float) -> int:
    ...
    for det in detections:
        ...
        mask = project_letterbox_mask_roi(mask, frame, (x1, y1, x2, y2))
        threshold = 0.5 * (255.0 if int(mask.max()) > 1 else 1.0)
